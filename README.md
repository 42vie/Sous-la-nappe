...
| AU-03 | 🔴 3 des 4 mini-jeux MVP référencés dans `data/scenes.json` (`tone_puzzle` scène 3, `dosage_order` scène 8, `audio_reconstruction` scènes 10/11) n'ont aucun composant. Seul `MorpionMinigame` (`tictactoe_hidden`, scène 5) est branché dans `SceneEngine.tsx`. | 🟡 En cours | Corrigé le 31/07 pour ne plus bloquer la partie : `SceneEngine` ignore les `minigameId` sans composant et affiche narration + choix à la place. Il manque toujours les 3 vraies UI de mini-jeux |
