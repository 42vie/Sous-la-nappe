...
| S4-06 | Playtests des 6 POV | ⬜ À faire | Débloqué (AU-01, AU-09) : un run complet des 11 scènes est maintenant jouable pour les 6 personnages, à playtester réellement |
...
| AU-02 | 🔴 Le moteur de déviation (`lib/engine/deviation.ts` — couches 1 à 4 du chapitre 12 : `resolveHostIntent`, `resolveSeatingVariant`, `resolveTargetActual`) n'est appelé nulle part dans le code. Un nouveau run n'a donc ni `hostIntent`, ni `seatingVariant`, ni cible calculée selon les probabilités de la bible — les valeurs restent celles codées en dur dans `INITIAL_CHARACTER_STATE`. | 🟡 En cours | `resolveHostIntent`/`resolveSeatingVariant` branchés dans `/api/run/new` le 31/07. `resolveTargetActual` (couche 4, résolution de la cible réelle au service) reste non branché — actuellement calculé implicitement par le contenu de scène plutôt que par le moteur probabiliste |
