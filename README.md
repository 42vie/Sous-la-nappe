...
| S4-02 | `app/api/run/[runId]/final/route.ts` — endpoint rapport + score | ✅ Fait | Créé le 31/07 : GET renvoie le rapport 3 colonnes + le quiz de reconstruction (8 questions, chapitre 14) ; POST calcule le score (barème `data/questions_final.json`, total 100 pts) et le persiste sur le run |
| S4-03 | Brancher indices critiques C-09, C-11, C-18, C-19, C-20, C-25 | ⬜ À faire | AU-01 résolu, débloqué. C-09, C-18 n'ont encore aucun choix qui les révèle dans `data/scenes.json` |
| S4-04 | Endings MVP (F1 canonique + D1 déviation) | 🟡 En cours | `endingCalculator.ts` calcule 8 fins et alimente maintenant un vrai écran final avec quiz. Les conditions/noms restent différents du canon bible chap. 13 — voir AU-05 |
| S4-05 | Tests cohérence seeds + moteur | ⬜ À faire | `npx ts-node scripts/validate-content.ts` |
| S4-06 | Playtests des 6 POV | ⬜ À faire | Un run complet des 11 scènes + écran final a été vérifié le 31/07 par simulation du moteur (hors Firebase, cet environnement n'a pas accès aux identifiants du projet) pour les 6 personnages, sans cul-de-sac ni erreur. Reste à faire : un vrai playtest humain en conditions réelles (navigateur + Firebase) |
...
| # | Tâche | Statut | Notes |
|---|---|---|---|
| AU-01 | 🔴 **Bloquant absolu** — `app/api/run/new/route.ts` écrit un `RunState` à l'ancien schéma plat (`id`, `playerCharacter`, `currentScene: 1` en number, `sceneChoices`, `characterState` au niveau racine) au lieu du schéma en couches attendu par le moteur (`canon` / `variable` / `subjective`, `playerPov`, `currentScene: SceneId` en string). | ✅ Fait | Corrigé le 31/07 : écrit le vrai schéma, réutilise `initBackgroundFlags()` |
| AU-02 | 🔴 Le moteur de déviation (`lib/engine/deviation.ts` — couches 1 à 4 du chapitre 12 : `resolveHostIntent`, `resolveSeatingVariant`, `resolveTargetActual`) n'est appelé nulle part dans le code. Un nouveau run n'a donc ni `hostIntent`, ni `seatingVariant`, ni cible calculée selon les probabilités de la bible — les valeurs restent celles codées en dur dans `INITIAL_CHARACTER_STATE`. | ✅ Fait | `resolveHostIntent`/`resolveSeatingVariant` branchés dans `/api/run/new` le 31/07. `resolveTargetActual` (couche 4) branché le 31/07 dans `applyChoice` à la scène du service critique — vérifié par simulation complète, `targetActual` n'est plus jamais vide dans le rapport final |
| AU-03 | 🔴 3 des 4 mini-jeux MVP référencés dans `data/scenes.json` (`tone_puzzle` scène 3, `dosage_order` scène 8, `audio_reconstruction` scènes 10/11) n'ont aucun composant. Seul `MorpionMinigame` (`tictactoe_hidden`, scène 5) est branché dans `SceneEngine.tsx`. | ✅ Fait | `ToneMinigame` (scène 3, lecture de tonalité) et `DosageMinigame` (scène 8, ordre de service) ajoutés le 31/07 — les deux impactent `socialTension` selon la réussite. `audio_reconstruction` reste volontairement sans UI : remplacé par les conversations entendues en texte (`lib/engine/overheard.ts`), pas par un mini-jeu. Au passage, correction d'un bug de fond dans `sceneRunner.ts` (`minigameToLaunch` référençait la scène qu'on quitte au lieu de celle qu'on rejoint — provoquait une boucle de relance du mini-jeu après le morpion) |
| AU-09 | 🔴 `data/scenes.json` n'avait de contenu écrit (`narrativeBlocks`, `choices`) que pour le POV Lucas — les 5 autres personnages n'avaient que le bloc `default` et aucun choix accessible (`availableFor: ['lucas']` partout), donc aucune progression possible. | ✅ Fait | Ajouté le 31/07 : texte narratif propre à chaque personnage sur les 11 scènes (voix et angles morts du chapitre 4 de la bible), `availableFor` élargi à `'all'` sur les choix génériques en conservant l'exclusivité Lucas sur ses indices propres (vocal C-20, reflet C-11) |
| AU-04 | `app/api/run/[runId]/final/route.ts` n'existe pas alors que `app/(game)/run/[runId]/final/page.tsx` le fetch au chargement. `buildFinalReport()` (`endingCalculator.ts`) n'est appelé par aucune route. | ✅ Fait | Créé le 31/07, avec le quiz de reconstruction (chapitre 14, 8 questions déjà présentes dans `data/questions_final.json` mais jamais utilisées jusqu'ici) |
...
| AU-06 | `data/clues.json` contient 25 indices (C-01 à C-25) ; le type `ClueId` (`lib/types/clues.ts`) en autorise 26 (jusqu'à C-26), mais aucun C-26 n'apparaît nulle part dans la bible ni dans les données. | ✅ Fait | Type corrigé le 31/07 pour s'arrêter à C-25, avec une note expliquant la coquille du document source |
...
| AU-08 | `lib/firebase/scores.ts` (sauvegarde de score) n'est appelé par aucune route ; rien n'écrit jamais dans la collection Firestore `scores`. | 🟡 En cours | Le score final est désormais persisté le 31/07, mais directement sur le document `runs` (`finalScore`/`finalAnswers`) plutôt que via `scores.ts`/la collection `scores`, plus simple pour un score par run. `scores.ts` reste inutilisé — utile seulement si on veut un historique de scores multi-runs par joueur |
| AU-10 | 🔴 Bug moteur : l'effet `seating_update` (changement de place, scènes 4/7/9) ne faisait rien — il réécrivait la valeur déjà présente sur elle-même au lieu d'appliquer le nouveau placement. Le plan de table (« mécanique centrale du jeu » selon la bible, chapitre 5) restait figé sur la disposition de base toute la partie, quels que soient les choix du joueur. | ✅ Fait | Corrigé le 31/07 dans `sceneRunner.ts` : mappe désormais `effect.value` (`'base'`/`'swapB'`) vers `BASE_SEATING`/`SWAP_B_SEATING` |
| AU-11 | 🔴 Bug moteur : le résultat du mini-jeu morpion (`serviceHelper`, "la variable la plus importante du jeu" selon la bible chapitre 8) n'était mis à jour que côté client (Zustand) — jamais renvoyé au serveur. Le prochain `advance()` relisait Firestore et perdait le résultat. `syncToFirestore()` existait dans le store mais n'était appelé nulle part dans le code. | ✅ Fait | Corrigé le 31/07 : `handleMinigameComplete` appelle `syncToFirestore()` immédiatement après le résultat du mini-jeu |
| AU-12 | 🔴 Bug d'affichage : `SceneEngine` comparait `activeMinigame === 'morpion'`, mais l'identifiant réel dans les données est `tictactoe_hidden` — cette condition n'était donc jamais vraie et le morpion (scène 5) ne s'affichait jamais, même une fois AU-11 corrigé. `MorpionMinigame` faisait aussi toujours jouer le personnage humain dans le rôle de Sarah, quel que soit le POV réellement incarné. | ✅ Fait | Corrigé le 31/07 : bon identifiant utilisé ; le joueur contrôle son propre personnage quand il incarne Sarah ou Yanis, sinon il contrôle Sarah (POV spectateurs) |
